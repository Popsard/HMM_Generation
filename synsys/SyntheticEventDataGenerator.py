...
class SyntheticEventGenerator():
    ...
    def __init__(self, datafilename, numhmmstatesactivity, numhmmstatessensors, timeinterval, timestampmodeltype, main_event):
    ...
    #create an HMM model for the activity sequence, fit to the real data
    def fitActivityHMM(self):
        realactivitysequencematrix = self.realactivitysequence.to_numpy()
    ...
    def generateActivitySequence(self, activity, numsamples):
        if(self.isinit):
            if(activity == None):#use default start state
                ...
                #print(self.activityhmmmodel.resample(activtiy, numsamples, path=True))
                #print(activity)
                #print(numsamples)
    ...
    def getMostLikelyActivitiesDict(self, data, timeinterval):
        ...
        for date in dates:
            ...
            while start <= end:
                ...
                mostfreqactivity = self.main_event
                if (len(window) != 0):
